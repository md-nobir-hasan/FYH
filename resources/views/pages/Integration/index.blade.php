            ...
            <div class="block rounded-lg bg-white text-center shadow-lg dark:bg-neutral-700">
                <div class="border-b-2 border-neutral-100 py-3 px-6 dark:border-neutral-600 dark:text-neutral-50">
                    <div class="flex justify-between">
                        <h4 class="text-bold text-lg">Integrations</h4>
                        @if ($n->add)
                            <a href="{{ route('admin.page.page.services.create') }}">
                             ...
                             <tbody>
                                @foreach ($integration as $key=>$item)
                                    <tr
                                        ...
                                        <th scope="row"
                                            class="px-6 py-2 border-r font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{$item->title}}
                                        </th>
                                         <th scope="row"
                                            class="px-6 py-2 border-r font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ Str::limit($item->description, 35)}}
                                         ...
                                         <th scope="row"
                                            class="px-6 py-2 border-r font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                             <img src="{{'/storage/'.$item->image}}" alt="" width="50px">
                                        </th>
                                         <th scope="row"
                                            class="px-6 py-2 border-r font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{$item->priority}}
                                        ...
                                        class="px-6 py-2 border-r font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        @if($item->status ==1)
                                        <a href="{{route('admin.page.integrations.status', $item->id)}}" class="bg-blue-800 px-4 py-2 rounded-lg text-white">Active</a>
                                        @else
                                        <a href="{{route('admin.page.integrations.status', $item->id)}}" class="bg-red-800 px-4 py-2 rounded-lg text-white">Dective</a>
                                        @endif
                                    </th>
                                        @if ($n->edit || $n->delete)
                                            <td class="px-6 py-2 text-center ">
                                                <div class="flex justify-center">
                                                    @if ($n->show)
                                                    <a href="{{route('admin.page.integrations.show',$item->id)}}" target="_blank"
                                                ...
                                                @endif
                                                @if ($n->edit)
                                                    <a href="{{route('admin.page.integrations.edit',$item->id)}}"
                                                ...
                                                @if ($n->delete)
                                                   {{-- <div class="delete"> --}}
                                                     <a href="{{route('admin.page.integrations.destroy',$item->id)}}"
